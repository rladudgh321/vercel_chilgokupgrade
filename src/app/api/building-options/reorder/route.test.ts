
import { POST } from './route';
import { NextRequest } from 'next/server';
import { createClient } from '@/app/utils/supabase/server';

// Supabase 클라이언트 모의(mock) 설정
jest.mock('@/app/utils/supabase/server', () => ({
  createClient: jest.fn(),
}));

describe('/api/building-options/reorder API 라우트', () => {
  let fromMock: any;
  let updateMock: any;
  let eqMock: any;

  beforeEach(() => {
    // 모든 모의 함수 초기화
    eqMock = jest.fn().mockResolvedValue({ error: null }); // 기본적으로 성공 응답
    updateMock = jest.fn(() => ({ eq: eqMock }));
    fromMock = jest.fn(() => ({ update: updateMock }));

    (createClient as jest.Mock).mockReturnValue({ from: fromMock });
  });

  describe('POST 핸들러', () => {
    test('유효한 ID 배열로 순서를 성공적으로 업데이트해야 합니다.', async () => {
      const orderedIds = [3, 1, 2];
      const req = new NextRequest('http://localhost', { 
        method: 'POST', 
        body: JSON.stringify({ orderedIds }) 
      });

      const response = await POST(req);
      const body = await response.json();

      expect(response.status).toBe(200);
      expect(body.ok).toBe(true);
      expect(fromMock).toHaveBeenCalledWith('BuildingOption');
      expect(updateMock).toHaveBeenCalledTimes(orderedIds.length);
      expect(eqMock).toHaveBeenCalledWith('id', 3);
      expect(updateMock).toHaveBeenCalledWith({ order: 0 });
      expect(eqMock).toHaveBeenCalledWith('id', 1);
      expect(updateMock).toHaveBeenCalledWith({ order: 1 });
      expect(eqMock).toHaveBeenCalledWith('id', 2);
      expect(updateMock).toHaveBeenCalledWith({ order: 2 });
    });

    test('orderedIds가 배열이 아니면 400 에러를 반환해야 합니다.', async () => {
      const req = new NextRequest('http://localhost', { 
        method: 'POST', 
        body: JSON.stringify({ orderedIds: 'not-an-array' }) 
      });

      const response = await POST(req);
      const body = await response.json();

      expect(response.status).toBe(400);
      expect(body.ok).toBe(false);
      expect(body.error.message).toBe('orderedIds must be an array.');
    });

    test('DB 업데이트 중 에러가 발생하면 400 에러를 반환해야 합니다.', async () => {
      const orderedIds = [3, 1, 2];
      // 특정 호출에서만 에러를 발생시키도록 설정
      eqMock.mockImplementation((field: string, value: any) => {
        if (value === 1) {
          return Promise.resolve({ error: { message: 'DB Error' } });
        }
        return Promise.resolve({ error: null });
      });

      const req = new NextRequest('http://localhost', { 
        method: 'POST', 
        body: JSON.stringify({ orderedIds }) 
      });

      const response = await POST(req);
      const body = await response.json();

      expect(response.status).toBe(400);
      expect(body.ok).toBe(false);
      expect(body.error).toBeDefined();
      expect(body.error.message).toBe('DB Error');
    });
  });
});
