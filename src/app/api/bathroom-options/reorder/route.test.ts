...
describe('/api/bathroom-options/reorder API 라우트', () => {
  let fromMock: any;
  let updateMock: any;
  let eqMock: any;
  ...
  describe('POST 핸들러', () => {
    ...
    test('DB 업데이트 중 에러가 발생하면 400 에러를 반환해야 합니다.', async () => {
      const orderedIds = [3, 1, 2];
      // Simulate a DB error for one of the updates
      const eqMockFn = (field: string, value: any) => {
        if (value === 1) {
          return Promise.resolve({ error: { message: 'DB Error' } });
        }
        return Promise.resolve({ error: null });
      };
      eqMock.mockImplementation(eqMockFn);
