...
describe('/api/room-options API 라우트', () => {
  let fromMock: any;
  let selectMock: any;
  let orderMock: any;
  let insertMock: any;
  ...
  // GET
  describe('GET 핸들러', () => {
    test('모든 RoomOption을 성공적으로 가져와야 합니다.', async () => {
      const mockData = [{ id: 1, name: '원룸' }, { id: 2, name: '투룸' }];
      ...
      expect(fromMock).toHaveBeenCalledWith('RoomOption');
    });
  });

  // POST
  describe('POST 핸들러', () => {
    test('새로운 RoomOption을 성공적으로 추가해야 합니다.', async () => {
      const newName = '쓰리룸';
    ...
    test('이미 존재하는 이름이면 400 에러를 반환해야 합니다.', async () => {
        ...
        const req = new NextRequest('http://localhost', { method: 'POST', body: JSON.stringify({ name: '원룸' }) });
        const response = await POST(req);
        const body = await response.json();
  ...
  // DELETE
  describe('DELETE 핸들러', () => {
    test('RoomOption을 성공적으로 삭제해야 합니다.', async () => {
      const idToDelete = '1';
